...
@testset "Adding data proggresively" begin
    ...
    @testset "add_bulk!" begin
        ...
        @test props(g, 3) == Dict(:q₀=>[0,1],:q₂=>[2,1])
        @test props(g, 4) == Dict(:q₀=>[0,2],:q₂=>[2,2])
        @test props(g, 5) == Dict(:q₀=>[0,3],:q₂=>[2,3])
        ...
        @test nv(g) == 8
        @test ne(g) == 7
        @test props(g, 6) == Dict(:B=>0.5)
        @test props(g, 7) == Dict(:q₀=>[0,-1],:q₂=>[2,-1])
        @test props(g, 8) == Dict(:q₀=>[0,-2],:q₂=>[2,-2])
